...
const parseItem = (item, resp) => {
  ...
  switch (type) {
    ...
    // New type & file without json until now => ignore instead of crash
    default:
      console.warn(`[ytsr] Unknown type encountered: ${type}. Skipping...`);
      return null;
  }
};
...
const parseVideo = obj => {
  ...
  try {
    author = obj.ownerText && obj.ownerText.runs && obj.ownerText.runs[0];
    if (author && author.navigationEndpoint) {
      const ep = author.navigationEndpoint;
      authorUrl = ep.browseEndpoint?.canonicalBaseUrl ||
        ep.commandMetadata?.webCommandMetadata?.url ||
        ep.browseEndpoint?.browseId ||
        '/';
    }
  } catch (err) {
    console.warn('[ytsr] Failed to parse author info, using fallback');
  }
  ...
  // Safe thumbnail overlays access
  const overlays = obj.thumbnailOverlays || [];
  const lengthFallback = overlays.find(x => x && Object.keys(x)[0] === 'thumbnailOverlayTimeStatusRenderer');
  ...
  return {
    ...
    bestThumbnail: prepImg((obj.thumbnail && obj.thumbnail.thumbnails) || [])[0],
...
const parseShelf = obj => {
  ...
  try {
    if (Array.isArray(obj.contents)) {
      rawItems = obj.contents
        .map(x => x?.richItemRenderer?.content || x?.videoRenderer || x?.playlistRenderer)
        .filter(x => x);
    } else if (Array.isArray(obj.items)) {
      rawItems = obj.items.filter(x => x);
    } else if (obj.content) {
      const list = obj.content.verticalListRenderer || obj.content.horizontalMovieListRenderer || obj.content.horizontalMovieListRenderer;
      if (list && Array.isArray(list.items)) {
        rawItems = list.items.filter(x => x);
      }
    }
  } catch (err) {
    console.warn('[ytsr] Shelf parsing failed:', err.message);
  }
